...
/*
 * highgo:
 * this action is to control logic when a node in highgo cluster startup,
...
do_node_startup(void)
{
    ...
    strcpy(check_cmd,"ps -eo pid,cmd|grep postgres|grep -qv grep");
    ret = system(check_cmd);
    if (ret == 0) //pg is up already
    {
        log_notice("The postgres is up already, quit node startup action.");
        exit(1);
    }

    snprintf(run_cmd,MAXPGPATH,"%s/postgresql.conf",
            config_file_options.data_directory);
    while(true)
    {
        ...
        {
            log_notice("%s/postgresql.conf not found, will check again after 5 seconds",
                    config_file_options.data_directory);
            sleep(5);
        }
    }
#if 0
    snprintf(run_cmd,MAXPGPATH,"%s/hgdb_log",
            config_file_options.data_directory);
    if(access(run_cmd,0) != 0) /* not exists */
    {
        snprintf(run_cmd,MAXPGPATH,"mkdir %s/hgdb_log",
            config_file_options.data_directory);
        system(run_cmd);
    }
#endif
    ...
    for(i=0;i<5;i++)
    {
        snprintf(run_cmd,MAXPGPATH,"%s/pg_ctl start -D %s -w -l /tmp/pglog",
                config_file_options.pg_bindir, config_file_options.data_directory);
        log_notice("start postgres by:%s/pg_ctl start -D %s -w -l /tmp/pglog",
                config_file_options.pg_bindir,config_file_options.data_directory);
        system(run_cmd);
        sleep(2);
        ...
        {
            pg_started = true;
            log_notice("start postgres successfully");
            break;
        }
    }
    if(!pg_started)
    {
        log_notice("failed to start postgres");
        exit(1);
    }
    ...
    snprintf(run_cmd,MAXPGPATH,"%s/repmgrd -d",config_file_options.pg_bindir);
    while(true)
    {
        system(run_cmd);
        sleep(10);
    ...
    snprintf(run_cmd,MAXPGPATH,"%s/standby.signal",
            config_file_options.data_directory);
    ...
    else
    {
        ...
        if(get_all_node_records(conn, &mynodes))
        {
            ...
            for(cell=mynodes.head;cell;cell=cell->next)
            {
                ...
                if (PQstatus(cell->node_info->conn) == CONNECTION_OK)
                {
                    ...
                    if(cell->node_info->recovery_type == RECTYPE_PRIMARY) /* find other primary node */
                    {
                        ...
                        snprintf(run_cmd,MAXPGPATH,"%s/pg_ctl -D %s stop -m fast",
                                config_file_options.pg_bindir, config_file_options.data_directory);
                        system(run_cmd);
                        PQfinish(conn);
                        exit(0);
            ...
            if(check_vip_conf(config_file_options.virtual_ip, config_file_options.network_card))
            {
                ...
                bind_virtual_ip(config_file_options.virtual_ip, config_file_options.network_card);
                PQfinish(conn);
                exit(0);
            }
        }
