...
repmgrd_set_pid(PGconn *conn, pid_t repmgrd_pid, const char *pidfile)
{
    ...
    appendPQExpBuffer(&query,
        "SELECT repmgr.set_repmgrd_pid(%i, ",(int) repmgrd_pid);
    
    if (pidfile != NULL)
    {
        appendPQExpBuffer(&query," '%s')",pidfile);
    }
    else
    {
 ...
 */
void
hg_get_all_node_records(PGconn *conn, NodeInfoList *node_list)
{
    PQExpBufferData query;
    PGresult   *res = NULL;
...
_create_update_node_record(PGconn *conn, char *action, t_node_info *node_info)
{
    ...
    param_values[11] = node_info->virtual_ip;   //highgo
    param_values[12] = node_info->network_card; //highgo
...
/*
 * highgo: Bind virutal IP to locale node network card
 * tianbing
 */
bool
bind_virtual_ip(const char *vip, const char *network_card)
{
    ...
    if(uid == 0) //root user
        ...
        sprintf(bind_vip, "sudo ip addr add %s dev %s", vip, network_card);
    r = system(bind_vip);
...
/*
 * highgo: Unbind virutal IP to locale node network card
 * tianbing
 */
bool
unbind_virtual_ip(const char *vip, const char *network_card)
{
    ...
    if(uid == 0) //root user
        ...
        sprintf(unbind_vip, "sudo ip addr del %s dev %s", vip, network_card);
...
/*
 * highgo: Check that if virutal ip, network card has configured
 * tianbing
 */
bool
...
/*
 *  highgo: get virtual ip
 *  tianbing
 */
bool
get_virtual_ip(PGconn *conn, int primary_id, char *virtual_ip)
{
    PQExpBufferData query;
    PGresult   *res = NULL;
    ...
    strncpy(virtual_ip, PQgetvalue(res, 0, 0), MAXLEN);
...
/*
 *  highgo: get network card
 *  tianbing
 */
bool
get_network_card(PGconn *conn, int primary_id, char *network_card)
{
    PQExpBufferData query;
    PGresult   *res = NULL;
    ...
    strncpy(network_card, PQgetvalue(res, 0, 0), MAXLEN);
...
get_pg_size_pretty(PGconn *conn, long long unsigned int lag_bytes, char *lag_str)
{
    ...
    appendPQExpBuffer(&query, "SELECT pg_catalog.pg_size_pretty(%llu::numeric)", lag_bytes);
    ...
    if (PQresultStatus(res) != PGRES_TUPLES_OK)
    {
        ...
        strcpy(lag_str, PQgetvalue(res, 0, 0));
