...
load_config(const char *config_file, bool verbose, bool terse, t_configuration_options *options, char *argv0)
{
	 ...
	 * in this order:
	 *  - HG_db default conf directory:$HG_HOME/conf/hg_repmgr.conf
	 *  - current directory
	 *  - /etc/repmgr.conf
	 *  - default sysconfdir
	 *
	...
	if (config_file_provided == false)
	{
        ...
        get_include_path(my_exec_path, home_path);
        *last_dir_separator(home_path) = '\0';

        if (verbose == true)
            fprintf(stdout, _("INFO: looking for configuration file in \"%s\"\n"), home_path);

        maxpath_snprintf(config_file_path, "%s/conf/%s", home_path, CONFIG_FILE_NAME);
        if (stat(config_file_path, &stat_config) == 0)
        {
            config_file_found = true;
            goto end_search;
...
_parse_config(t_configuration_options *options, ItemList *error_list, ItemList *warning_list)
{
	...
	memset(options->config_directory, 0, sizeof(options->config_directory));
    ...
    /*-----------------
     * highgo: Virtual IP control settings
     * -----------------
     */
    memset(options->virtual_ip, 0, sizeof(options->virtual_ip));
    memset(options->network_card, 0, sizeof(options->network_card));

    /*-----------------
     * highgo: Device check settings
     * -----------------
     */
    options->device_check_timeout = DEVICE_CHECK_TIMEOUT;
    options->device_check_times = DEVICE_CHECK_TIMES;

    /*-----------------
     * highgo: Standby wait timeout
     *-----------------
     */
    options->standby_wait_timeout = DEFAULT_STANDBY_WAIT_TIMEOUT;

    /* by default do not check brain split */
    options->check_brain_split = false;
	...
	while ((s = fgets(buf, sizeof buf, fp)) != NULL)
	{
        ...
        /* highgo: vitural ip settings */
        else if (strcmp(name, "virtual_ip") == 0)
            strncpy(options->virtual_ip, value, MAXLEN);
        else if (strcmp(name, "network_card") == 0)
            strncpy(options->network_card, value, MAXLEN);

        /* highgo: device check settings */
        else if (strcmp(name, "device_check_timeout") == 0)
            options->device_check_timeout = repmgr_atoi(value, name, error_list, 1);
        else if (strcmp(name, "device_check_times") == 0)
            options->device_check_times = repmgr_atoi(value, name, error_list, 0);

        /* highgo: standby wait timeout */
        else if (strcmp(name, "standby_wait_timeout") == 0)
            options->standby_wait_timeout = repmgr_atoi(value, name, error_list,1);
        /* highgo: check brain split */
        else if (strcmp(name, "check_brain_split") == 0)
            options->check_brain_split = parse_bool(value, name, error_list);
...
reload_config(t_configuration_options *orig_options, t_server_type server_type)
{
    ...
    /* check_brain_split */
    if (orig_options->check_brain_split != new_options.check_brain_split)
    {
        orig_options->check_brain_split = new_options.check_brain_split;
        log_info(_("\"check_brain_split\" is now \"%s\""),
                new_options.check_brain_split == true ? "TRUE" : "FALSE");
        config_changed = true;
    }
...
modify_auto_conf(const char *data_dir, KeyValueList *items)
{
    ...
    for (cell = config.head; cell; cell = cell->next)
    {
        appendPQExpBuffer(&auto_conf_contents,
                "%s = '%s'\n",
                cell->key, cell->value);
        log_debug("%s",auto_conf_contents.data);
    }
