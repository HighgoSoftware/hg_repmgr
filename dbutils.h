...
typedef struct s_node_info
{
    ...
    char        virtual_ip[MAXPGPATH];   //highgo
    char        network_card[MAXPGPATH]; //highgo
    ...
    /* highgo: record counts that failed connect to node */
...
void        hg_get_all_node_records(PGconn *conn, NodeInfoList *node_list); //highgoDB added
...
/*highgo:*/
void get_pg_size_pretty(PGconn *conn, long long unsigned int lag_bytes, char *lag_str);
/* highgo: VIP manager functions */
...
bool get_virtual_ip(PGconn *conn, int primary_id, char *virtual_ip);
bool get_network_card(PGconn *conn, int primary_id, char *network_card);
