...
do_primary_register(void)
{
    ...
    char    virtual_ip[MAXLEN] = ""; //highgo
    char    network_card[MAXLEN] = ""; //highgo
    bool    old_vip_found = false; //highgo
	...
	{
        /*record the virtual ip and network card for unbind */
        old_vip_found = get_virtual_ip(conn, config_file_options.node_id, virtual_ip) && get_network_card(conn, config_file_options.node_id, network_card);
	...
	if (record_status == RECORD_FOUND)
	{
        /* highgo: Force register primary, unbind the old virtual ip */
        if (old_vip_found)
        {
            unbind_virtual_ip(virtual_ip, network_card);
        }

        /* Then, rebind the new virtual ip */
	...
	{
        /* highgo: When primary node has registerd successful, bind the virtual ip to node's network card*/
        if(check_vip_conf(config_file_options.virtual_ip, config_file_options.network_card))
        {
            bind_virtual_ip(config_file_options.virtual_ip, config_file_options.network_card);
        }
