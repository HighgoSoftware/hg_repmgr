	...
	if (repmgr_command != NULL)
	{
		...
		else if (strcasecmp(repmgr_command, "NODE") == 0)
		{
			...
			if (strcasecmp(repmgr_action, "CHECK") == 0)
				...
				action = NODE_CONTROL;
            else if (strcasecmp(repmgr_action, "STARTUP") == 0)
                action = NODE_STARTUP;
		}
...
init_node_record(t_node_info *node_record)
{
    ...
    /* highgo: init the virtual ip */
    strncpy(node_record->virtual_ip, config_file_options.virtual_ip, MAXPGPATH);
    strncpy(node_record->network_card, config_file_options.network_card, MAXPGPATH);
